...
class Charges extends Connection
{
    ...
    {
        return $this->genericQuery("SELECT * FROM charges WHERE client_id = $client_id AND id != $charge_id AND NOT payed ORDER BY id DESC;", true);
    }
    ...
    {
        return $this->genericQuery("SELECT * FROM services WHERE id IN (SELECT DISTINCT service_id FROM charge_details WHERE charge_id = $charge_id)", true);
    }
